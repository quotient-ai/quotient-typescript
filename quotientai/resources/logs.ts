...
export class LogsResource {
    ...
    private convertToDetectionResults(response: DetectionResultsResponse): DetectionResults {
        ...
        const logDocuments = response.log_documents?.map(doc => {
        ...
        const logMessageHistory = response.log_message_history?.map(msg => {
        ...
        const logInstructions = response.log_instructions?.map(inst => {
        ...
        const evaluations = response.evaluations.map(evalItem => {
            ...
            const documentEvaluations = evalItem.document_evaluations.map((docEval: DocumentEvaluationResponse) => {
            ...
            const messageHistoryEvaluations = evalItem.message_history_evaluations.map((msgEval: MessageHistoryEvaluationResponse) => {
            ...
            const instructionEvaluations = evalItem.instruction_evaluations.map((instEval: InstructionEvaluationResponse) => {
