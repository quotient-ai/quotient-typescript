...
describe('BaseQuotientClient', () => {
  ...
  it('should update auth header with valid token', () => {
    ...
    expect(privateClient.client.defaults.headers.common['Authorization']).toBe(
      'Bearer valid_token'
    );
  ...
  it('should handle GET requests correctly', async () => {
    ...
    expect(privateClient.client.get).toHaveBeenCalledWith('/test', {
      params: { param: 'value' },
      timeout: 1000,
    });
  ...
  it('should handle POST requests with array data', async () => {
    ...
    expect(privateClient.client.post).toHaveBeenCalledWith('/test', ['value1', 'value2'], {
      timeout: 1000,
    });
    expect(result).toBe(mockResponse.data);
  });
  ...
  it('should handle DELETE requests', async () => {
    ...
    expect(privateClient.client.delete).toHaveBeenCalledWith('/test', { timeout: 1000 });
    expect(result).toBe(mockResponse.data);
  });
});
